...
const Message = ({ content, isUserMessage }: MessageProps) => {
  return (
    <div className={cn({
      ...
      <div className='p-6'>
        <div className='max-w-3xl mx-auto flex items-start gap-2.5px'>
          ...
          <div className='flex flex-col ml-6 w-full'>
            <div className='flex items-center space-x-2'>
              ...
              <span className='text-sm font-semibold text-gray-900 dark:text-white'>{isUserMessage ? "You" : "AI"}

              </span>
            </div>
            <p className='text-sm font-normal py-2.5px text-gray-900 dark:text-white'>{content}</p>
          </div>
        </div>
      </div>
    </div>
